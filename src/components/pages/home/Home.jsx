...
export const Home = () => {
  ...
  const { userAddress, vwbl, web3, connectWallet, disconnectWallet } =
    VwblContainer.useContainer() // web3,vwblを追加
  ...
  const fetchNfts = async () => {
    ...
    if (!userAddress || !web3 || !vwbl) {
      console.log(
        'Now your wallet is not connected. Please connect your wallet.'
      )
      return
    }
        ...
        <>
          ...
          <Tabs selectedIndex={tabIndex} onSelect={index => setTabIndex(index)}>
            <TabList className='Tab-List'>
              <Tab className={clsx('Tab', tabIndex === 0 && 'isSelectedTab')}>
                Created
              </Tab>
              <Tab className={clsx('Tab', tabIndex === 1 && 'isSelectedTab')}>
                Owned
              </Tab>
            </TabList>
            <TabPanel
              className={tabIndex === 0 ? 'Tab-Panel' : 'isNotSelectedTab'}
            >
              <ItemList nfts={mintedNfts} />
            </TabPanel>
            <TabPanel
              className={tabIndex === 1 ? 'Tab-Panel' : 'isNotSelectedTab'}
            >
